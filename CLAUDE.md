...
Gameplay animation state lives in `TacticalMapScreen` and is fed from two places: `GameEffect` flows for one-off events the engine reports (`combatEvents`, `shakeEvents`), and diffs of successive `GameState`s for everything the AI does — an AI turn replaces the state wholesale and emits no effects, so moves, planet captures and sieges are *only* visible as a diff. Anything driven off a diff has two obligations: filter it against `visibleHexes` (an animation drawn from an unobserved hex leaks the fog of war — see `visiblePathSuffix`), and clear its state in a `finally`, guarded by reference identity, because a new state cancels the running `LaunchedEffect` and a half-finished animation otherwise leaves fleets permanently undrawn or hexes permanently veiled.
