...
- **Themes** straddle two modules. `:core:domain/theme` owns everything pure and tested — `ThemeType`, the `@Serializable` theme model, `ThemeParser`, `HexColor`, and `ThemeResolver` (preference × seasonal calendar). `:app/ui/theme` owns only what needs Android: reading `assets/themes/<type>.json`, building the `ColorScheme`, and `ThemePreferenceStore` (SharedPreferences). The theme is **not** part of `GameState` — it's a user preference, exposed by `GameViewModel.themePreference` as a `ThemeType?` where `null` means "seasonal". Resolve it once in `NovaEmpireTheme`; consumers read `LocalThemeType` / `LocalGraphicsConfig` rather than calling `ThemeManager` themselves. Adding a theme = one `ThemeType` entry + one JSON; `ShippedThemesTest` in `:core:domain` validates the files CI ships.
